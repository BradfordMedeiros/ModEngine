...
float waveLeftandRight = 0.05;
float waveHeight = 0.3;
float waveForwardTurb = 0.1;
float waveSpeed = 1;
vec3 calcWaveOffset(vec3 position){
	return position + vec3(
    waveForwardTurb * cos(position.x + (waveSpeed * time)), 
    (-1 * waveHeight)- (waveHeight * cos((position.x * 0.4)  + (waveSpeed * time))), 
    waveLeftandRight  * cos(position.x + (waveSpeed * time))
  );
}
