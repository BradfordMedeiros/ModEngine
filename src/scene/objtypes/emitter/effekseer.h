#ifndef MOD_EFFEKSEER
#define MOD_EFFEKSEER

#include <iostream>
#include <codecvt>
#include <Effekseer.h>
#include <EffekseerRendererGL.h>
#include "../../../common/util.h"

void initEffekseer();
void onEffekSeekerFrame(float timeElapsed, float currentTime, bool paused);
void onEffekSeekerRender(float windowSizeX, float windowSizeY, float fovRadians,  glm::vec3 viewPosition, glm::quat viewDirection, float nearPlane, float farPlane);

struct EffekEffect {
	objid effectId;
};

EffekEffect createEffect(std::string effec, glm::vec3 position, glm::quat rotation, glm::vec3 scale);
void freeEffect(EffekEffect& effect);
void setEffectState(EffekEffect& effect, bool loopContinuously);
void playEffect(EffekEffect& effect, glm::vec3 position);
void stopEffect(EffekEffect& effect);
void updateEffectPosition(EffekEffect& effectEffect, glm::vec3 position, glm::quat rotation, glm::vec3 scale);

void reloadEffect(std::string file);

void setEffectColor(EffekEffect& effectEffect, glm::vec4 color);

void playEffectOneShot(EffekEffect& effect, std::optional<glm::vec3> position);

int effekSeekerTriangleCount();
int effekSeekerDrawCount();

#endif
