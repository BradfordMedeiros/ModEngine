...
void physicsTranslateSet(World& world, objid index, glm::vec3 pos){
  auto parentMatrix = world.idToParentModel.at(index);
...
void traverseScene(World& world, Scene& scene, std::function<void(objid, glm::mat4, glm::mat4, bool)> onObject){
  traverseScene(world, scene, glm::mat4(1.f), glm::vec3(1.f, 1.f, 1.f), [&world, &onObject](objid id, glm::mat4 model, glm::mat4 parent, bool isOrtho) -> void {
    world.idToParentModel[id] = parent;
    onObject(id, model, parent, isOrtho);
  });
}
