...
void traverseScene(SceneSandbox& sandbox, Scene& scene, glm::mat4 initialModel, glm::vec3 scale, std::function<void(objid, glm::mat4, glm::mat4, bool, std::string, glm::vec3)> onObject){
...
Transformation fullTransformation(SceneSandbox& sandbox, objid id){
  ...
  traverseScene(sandbox, scene, [id, &foundId, &transformation](objid traversedId, glm::mat4 model, glm::mat4 parent, bool isOrtho, std::string fragshader, glm::vec3 tint) -> void {
    if (traversedId == id){
      foundId = true;
