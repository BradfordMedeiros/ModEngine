    ...
    if (meshObj != NULL && shouldRenderMesh(*meshObj, viewportId) && (meshObj -> meshesToRender.size() > 0)){
      ...
      for (int x = 0; x < meshObj -> meshesToRender.size(); x++){
        ...
        MeshUniforms meshUniforms {
          ...
          .cubemapReflection = meshObj -> cubemapReflection,
          .customTextureId = static_cast<unsigned int>(meshObj -> texture.loadingInfo.textureId),
          .customUseTextureId = static_cast<unsigned int>(meshObj -> customTexture.textureId),
          .customNormalTextureId = static_cast<unsigned int>(meshObj -> normalTexture.textureId),
          .customCubemapTextureId = static_cast<unsigned int>(meshObj -> cubemapTexture.textureId),
          .customOpacityTextureId = static_cast<unsigned int>(meshObj -> opacityTexture.textureId),
          .tint = meshObj -> tint,
