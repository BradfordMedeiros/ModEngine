...
Mesh loadMesh(std::string defaultTexture, MeshData modelData, std::function<Texture(std::string)> ensureLoadTexture, const char* debugHint);		 // loads model and returns mesh/bound texture data
...
struct MeshUniforms {
  ...
  glm::vec3 cubemapReflection = glm::vec3(0.f, 5.f, 0.2f);
  unsigned int customTextureId = -1;
  unsigned int customUseTextureId = -1;
